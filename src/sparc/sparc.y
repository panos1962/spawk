...
// σύμφωνα με τον global counter `input_count' ο οποίος αυξάνεται κατά
// ένα ακριβώς σ'αυτό το σημείο.

static void input_push(VARIABLE *v) {
	if (!v)
	sparc_locfatal("input_push: null variable pointer (internal error)", EXIT_FATAL);
...
static PARAMETER *cur_plist = NULL;
static PARAMETER *last_prm = NULL;
static PARAMETER *awk_plist = NULL;
...
static void push_prm(char *pname) {
	...
	if (!(prm = (PARAMETER *)malloc(sizeof(PARAMETER))))
	sparc_nomemory("push_prm");
...
static VARIABLE *set_var_type(VARIABLE *v, int typ, DB_COLUMN *col) {
	if (!v)
	sparc_locfatal("set_var_type: null variable pointer (internal error)", EXIT_FATAL);

	if (typ && col)
	sparc_locfatal("set_var_type: both type and column specified (internal error)", EXIT_FATAL);

	if ((!typ) && (!col))
	sparc_locfatal("set_var_type: neither type nor column specified (internal error)", EXIT_FATAL);

	if (
		(typ &&
			(
				(v->type && (v->type != typ)) || (v->column != NULL)
			)
		) ||
		((col != NULL) &&
			(
				v->type || ((v->column != NULL) && (v->column != col))
			)
		)
	) {
		yyerror(NULL);
...
static void awk_function(char *fname, TEXT *code) {
	...
	if (awk_plist != NULL) {
		...
		for (prm = awk_plist->next; prm; prm = prm->next) {
...
input
	...
	| SORTED_KWD INPUT_KWD ilist {
		if (!sorted_input)
		sparc_locerror("previous input considered sorted also");
...
slist
	...
	| slist variable asc_dsc '[' VARTYPE ']' {
		yyerrok;
		sort_push(set_var_type($2, $5, NULL), $3);
	}
...
aacs
	...
	| aacs '+' AACS {
		...
		if (($$ = (char *)malloc(strlen($1) + strlen($3) + 1)) == NULL)
		sparc_nomemory("concatenating aacs strings");
...
pplist
	: '(' {
		if (plist_na)
		sparc_locerror("parameters not allowed (ignored)");
	} prm_list ')' {
		$$ = (plist_na ? NULL : $3);
